...
def create_inventory_table():
    try:
        ...
        if table_exists:
           ...
           FOREIGN KEY (restaurantName) REFERENCES restaurant(restaurantName) ON DELETE CASCADE ON UPDATE CASCADE, FOREIGN KEY (itemID) REFERENCES item(itemName) ON DELETE CASCADE ON UPDATE CASCADE)
            ''')
            conn.commit()
